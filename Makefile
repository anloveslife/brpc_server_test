...
NEED_GPERFTOOLS=0
#此处定义brpc代码的目录
BRPC_PATH=/root/brpc_server/incubator-brpc
...
SERVER_SOURCES = $(wildcard src/service/*.cpp)
...
ifneq ("$(LINK_SO)", "")
	...
	rm ${SERVER_OBJS} ${PROTO_OBJS}

%.pb.cc %.pb.h:%.proto
	@echo "> Generating $@"
	$(PROTOC) --cpp_out=./src/proto --proto_path=./src/proto $(PROTOC_EXTRA_ARGS) $<
